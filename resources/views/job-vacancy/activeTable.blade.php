...
<table class=" min-w-full divide-y divide-gray-200 bg-white rounded-lg shadow mt-4 ">
  ...
  <tbody>
    <!-- row 1 -->
    @forelse ($jobVacancies as $index => $jobVacancy)
      <tr class="border-b *:px-6 *:py-4 *:text-left *:text-gray-800 *:border-r ">
        ...
        <td>
          <a class=" text-blue-500 hover:text-blue-700 hover:underline"
            href="{{ route('job-vacancies.show', ['job_vacancy' => $jobVacancy, 'tap' => 'company']) }}">
            {{ $jobVacancy->title }}
          </a>
        </td>
        ...
        <td class="!border-r-0 flex gap-x-6">
          {{-- Edit button --}}
          <a href="{{ route('job-vacancies.edit', $jobVacancy->id) }}" class="text-blue-500 hover:text-blue-700 ">
            🖊
