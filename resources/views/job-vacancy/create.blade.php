  ...
  <x-base-wrapper>
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded shadow-md">
      ...
      <div class="leading-3 mb-4">
        ...
        <h3 class="text-xl font-bold">New Job Vacancy
        ...
        <div class="mb-4 p-6 bg-gray-50 border border-gray-200 rounded shadow-sm">
          ...
          <div class="my-4">
            ...
            <select id="jobCategoryId" name="jobCategoryId"
              ...
              @foreach ($jobCategories as $jobCategory)
                <option value="{{ $jobCategory->id }}" {{ old('jobCategory') == $jobCategory->id ? 'selected' : '' }}>
                  {{ $jobCategory->name }}
                </option>
              @endforeach
            </select>
          ...
          <div class="my-4">
            ...
            <textarea type="text" id="description" name="description"
              class="{{ $errors->has('description') ? 'outline-red-500 outline outline-1' : '' }} h-40 shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description') }}</textarea>
            @error('description')
              <p class="text-red-500 text-sm mt-2">* {{ $message }}</p>
            @enderror
          </div>
