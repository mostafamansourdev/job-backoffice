<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800">
      {{ __('Edit company') . ' - ' . $company->name }} </h2>
  </x-slot>
      ...
      <form
        ...
        <div class="mb-4 p-6 bg-gray-50 border border-gray-200 rounded shadow-sm">
          ...
          <div class="my-4">
            ...
            <select id="industry" name="industry" value="{{ old('industry', $company->industry) }}"
        ...
        <div class="mb-4 p-6 bg-gray-50 border border-gray-200 rounded shadow-sm">
          ...
          {{-- owner email cannot be changed (read only) --}}
          <div class="my-4">
            <label for="owner_email" class="block text-gray-700 font-bold mb-2">Email
              <span class="font-normal text-gray-500 text-sm ">(Read only)</span>
            </label>
            <input disabled type="email" id="owner_email" name="owner_email"
              value="{{ old('owner_email', $company->owner->email) }}"
              class="{{ $errors->has('owner_email') ? 'outline-red-500 outline outline-1' : '' }}  shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-slate-100">
            @error('owner_email')
              <p class="text-red-500 text-sm mt-2">* {{ $message }}</p>
            @enderror
          </div>
          ...
          <div>
            <label for="owner_password" class="block text-gray-700 font-bold mb-2">Password
              <span class="font-normal text-gray-500 text-sm ">(Leave blank to keep the same)</span></label>
